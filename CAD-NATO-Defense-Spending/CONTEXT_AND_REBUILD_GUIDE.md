...
7b. **CAD-NATO-Analysis is local-only (user-set, July 19, 2026).** The `CAD-NATO-Analysis/` folder (long-form analysis papers) is never pushed to GitHub, and the report must NEVER reference, name, or link to it or its papers. The analyses may inform report prose as research input only, with claims re-anchored to their official sources.
...
15. **Post-Ankara-Summit update (July 8–9, 2026)** — added a new `⚡ Latest Developments: July 8, 2026` block covering the concluded Ankara Summit; moved `id="latest"` from the July 6 block to this new one (the July 6 h2 kept its `.latest-header` styling but lost the anchor). Content is sourced from official readouts (nato.int Ankara Summit Declaration + Rutte transcript; pm.gc.ca partnership/DSRB/Carney-Merz-Støre releases; national government sites) and reviews **all 32 members plus partners** (Ukraine, South Korea, EU, IP4) via parallel research agents. New block includes a 32-row member readout `data-table` (Member / At Ankara / 2026 defence %GDP / position). Key confirmed facts: Ankara Declaration (ironclad Article 5, Russia "long-term threat", €70B Ukraine 2026 + equivalent 2027, Iran, >$139B core rise, >$50B procurements, no new 5% milestone, next summit Albania); July 7 Defence Industry Forum (11-nation Saab GlobalEye AWACS-replacement on Bombardier airframe incl. Canada; Triton; 7-nation A400M pool; Poland $988M PAC-2; UK-led 12-nation >$50B deep-strike; HALO/Northlink Arctic satcom); Canada deliverables (DSRB launched with 9 founders + Canadian HQ; Telesat ESCP-P Arctic satcom; ~C$800M Kongsberg JSM; 212CD trilateral w/ Germany+Norway; hosts 2027 forum); NATO July 7 spending estimates (LT 5.33 / EE 5.1 / LV 4.92 / PL 4.68 / GR 3.65 above 3.5% core; US 3.17, DE 2.69, UK 2.56, FR 2.22); Trump disruption (Spain "wasted cause"/trade cut, Greenland demand, Iran memo "over", "Trump trillion"). Also refreshed stale pre-summit language: header eyebrow `June 2026`→`July 2026`; catalyst-calendar NATO Summit row → concluded/GlobalEye confirmed; Program Timeline CPSP/summit row → resolved; Live Source Tracker NATO Summit card → concluded with outcomes. Markdown source got a parallel condensed July 8 block (note: the `.md` remains behind the HTML for the June 28/29 + July 6 blocks, which were HTML-only in prior edits). Full QA re-run: PASS (tags balanced, no dup element ids, nav targets resolve, scripts balanced, Chart.js once + 2 charts, all `target=_blank` have `rel=noopener`, no web fonts). Note: the July 6 dated block's "Euronews previews" line (200 Patriot missiles, $12.8B Arctic satcom) was left as a dated preview; the July 8 block carries the corrected confirmed figures ($988M PAC-2; Northlink/HALO). Per user style memory, the new prose avoids em dashes.
16. **"Washington vs. Buy European" callout (July 2026)** — added a highlighted callout to the **EU Spend Overview** section (`id="euspend"`), inserted **immediately after the section's `🔑 Key Finding` callout**, before the `<hr />` and the `Five Observations` h3. It covers the Feb 19, 2026 POLITICO story that the US DoD lobbied the EU (via a submission to European Commission consultation #15753 on overhauling the 2009 EU Defence Procurement Directive) against "European preference," threatening to review the "Buy American" waivers ~19 EU states hold; the EU holding firm on SAFE's 35% non-EU content cap and design-authority clause; and the Canadian-lens contrast (Canada is the first non-European country admitted to SAFE, partnership Jun 2025 / participation agreement Feb 2026 / Council adoption Jun 2026, so it sits *inside* the Buy-European architecture the US is lobbying to loosen, while the US, holding no Security & Defence Partnership, is outside it). **Styling:** reused the existing `.callout callout-note` class with an inline `border-left:4px solid var(--amber)` accent and `color:var(--text)` (so it reads as a full-weight highlighted box, not the muted note default), deliberately NOT `.callout-kf` (reserved for key findings) or `.callout-warn` (reserved for the investment disclaimer). **Sourcing tiers:** CONFIRMED = US Mission to the EU post (~Mar 23, 2026) + Ambassador Puzder on record + the SAFE/EDIP regulation texts (EUR-Lex 2025/1106 Art. 16(10)/16(3)/16(11); 2025/2643 Art. 27) + Canada-SAFE dates (official EU); REPORTED = the exact DoD submission wording (lives in the EU consultation file, not a US press release); NOT SUPPORTED (excluded) = any separate State/Commerce/USTR action or Lockheed/RTX attribution. Four inline official links added (POLITICO, US Mission to EU, EC consultation #15753, EUR-Lex SAFE), all `target=_blank rel=noopener`. Parallel blockquote note added to the markdown EU section. No new CSS, no nav/structural change; the callout sits inside the EU section body so section-collapse wraps it automatically. Per user style memory, no em dashes. Full QA re-run: PASS.

17. **Post-summit follow-through update (July 19, 2026)**: added a new `⚡ Latest Developments: July 19, 2026` block on top; moved `id="latest"` from the July 8 header to it (July 8 h2 keeps `.latest-header`, loses the anchor). Fourteen flagged entries + a `callout-kf`, sourced via three parallel research agents (Canada / Europe / Asia) covering July 8–19. CONFIRMED anchors: GDLS-Canada first Strategic Partner + ~C$2B/190 ACSVs (PMO, Jul 16); Saudi defence-cooperation framework + 13 agreements >$1B (PMO, Jul 9); Canada third co-Framework Nation for MND-North (DND joint statement, Jul 8); Telesat ESCP-P Mil-Ka formalization, definitive contract "coming weeks" (Telesat, Jul 8); Paris Coalition of the Willing summit + 16 Rafale + 4 SAMP/T NG via EU loan + 10-nation ABM coalition without Canada (Élysée/Defense News, Jul 13); Macron +€36B/5yr; Merz Tomahawk confirmation (Jul 9); Bundestag €6.3B four TKMS F128 frigates (Jul 8) + Saab SEK 8.7B combat systems (Jul 16); SAFE first disbursements incl. Croatia €255M (EC, Jul 10); Rheinmetall first Werk-Niedersachsen 155mm to Ukraine (Jul 14); C$29.6M Mirabel UxS hub (Jul 14); CAF operations rebalance (Jul 17); Taiwan ATACMS $439.4M (Jul 15); Pitch Black 26 (Jul 16). REPORTED (labelled as such in-text): Canada GCAP observer, announcement expected Jul 21 Farnborough (no Ottawa confirmation); Türkiye DSRB founding-member confirmation (Reuters-sourced, Jul 13); CPSP Ottawa supplier ripples (OBJ, Jul 13); Hanwha Hamilton unwind + Korea "NATO wall" pattern (JoongAng/sedaily); Hormuz crisis incl. floated-and-withdrawn 20% toll (no official US document); France's SCALP/Aster/AASM production licences. Secondary touch-ups: watch-thread cards (CPSP, Fighter incl. GCAP-observer note, NATO Summit follow-through), "Last curated" → July 19; catalyst table (fighter row "~Nov 2026 reported"; new Telesat ESCP-P row); Program Timeline new Jul 9–16 row. **Companion-papers rule (user-set, standing):** the `CAD-NATO-Analysis/` folder is LOCAL-ONLY and is never pushed to GitHub, so the report must never link to it or mention the papers; an initial cross-linking pass (tracker subsection + three inline references) was added and then fully removed on user instruction. The analyses may inform report content as research input only (e.g. the MND-North brigade-vs-division clarification and the Telesat space-stack framing survive as plain prose with official sources). Markdown source got a parallel condensed July 19 block (colon-style header, honouring the no-em-dash rule; older block headers keep their legacy em dashes). Per user style memory, no em dashes in any new prose. Full QA re-run: PASS. **Correction (same day, user-flagged):** the block's SAFE paragraph initially claimed Canada had "no announced awards yet" under SAFE; this was wrong. The first Canadian SAFE award (Montréal's Marconi Technologies + Enamor International, >C$10M, ORION X650 tactical radios for Polish Cyber Command) was announced June 15, 2026 in a PMO *readout* at the G7 in Évian, before the July 8-19 research window, so the window-scoped agents never saw it and the negative claim went unverified. Paragraph rewritten around the PMO readout (CONFIRMED: "the first contract awarded to a Canadian company under SAFE, with many more to come") + Marconi Newswire release; a "Canada-EU SAFE Execution" watch-thread card and readout-feed sources were added, and the `/cad-defense-update` command gained a backfill rule, an all-32-member official-readout sweep with a feed roster, and a negative-claim verification rule.
